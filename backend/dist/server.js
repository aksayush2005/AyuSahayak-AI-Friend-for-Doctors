...
const mcp = new Client({ name: "frontend-client", version: "1.0.0" }, { capabilities: { tools: {} } });
...
app.post("/api/generate_prescription", async (req, res) => {
    ...
    try {
        ...
        if (!prescription) {
            ...
            const similarText = ragResp.content[0].text.trim();
            const prompt = `
...
app.post("/api/save_prescription", async (req, res) => {
    ...
    if (!patient_id || !symptoms || !prescription)
        return res.status(400).send("Missing inputs");
    try {
        await mcp.callTool({
