...
const PrescriptionModel = model<PrescriptionLog>(
  "PrescriptionLog",
  new Schema<PrescriptionLog>({
    id:           { type: String, required: true },
...
server.registerTool(
  ...
  async ({ patient_id }) => {
    ...
    const text = logs
      .map(h =>
        `Patient: ${h.patient_id} | Age: ${h.age} | Diagnosis: ${h.diagnosis} | History: ${h.history.join(
          ", "
        )} | Symptoms: ${h.symptoms} | Prescription: ${h.prescription}`
      )
      .join("\n");
...
server.registerTool(
  ...
  async ({ patient_id, symptoms, prescription, doctor_id }) => {
    ...
    await PrescriptionModel.create({
      ...
      doctor_id,
      doctor_name: doctorNames[doctor_id as keyof typeof doctorNames] || 'Unknown Doctor',
      age:          patient.age,
...
server.registerTool(
  ...
  async ({ patient_id, symptoms }) => {
    ...
    // 3) map into typed-with-vector objects
    type RecWithVec = typeof rawRecords[0] & { vector: number[] };
    const records: RecWithVec[] = rawRecords.map(r => ({ ...r, vector: [] }));

    // 4) compute embeddings
    await Promise.all(
      records.map(async r => {
        r.vector = await embed(
          `${r.age} ${r.diagnosis} ${r.history.join(" ")} ${r.symptoms} ${r.prescription}`
        );
      })
    );
    ...
    const queryVector = await embed(
      `${patient.age} ${patient.diagnosis} ${patient.history.join(" ")} ${symptoms}`
    );
    ...
    const rankedText = records
      ...
      .slice(0, 2)
      .map(x =>
        `Patient: ${x.r.patient_id} | Symptoms: ${x.r.symptoms} | Prescription: ${x.r.prescription}`
      )
      .join("\n");
